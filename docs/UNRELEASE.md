...
- Asset local de la librería `panzoom@9.4.3` (`wwwroot/lib/panzoom/panzoom.min.js`) incluido en `PolyglotCLI.web` y duplicado en `PolyglotCLI.Maui/wwwroot/` para soportar la versión desktop (Blazor WebView) sin dependencia del CDN.
- Módulo JS de interoperabilidad `wwwroot/js/panzoomInterop.js` que adapta la librería `panzoom` para inicializar y manipular instancias por elemento DOM desde Blazor mediante `IJSRuntime.InvokeAsync<IJSObjectReference>("import", "./js/panzoomInterop.js")`.
...
- Verificador de Páginas (Web + Desktop MAUI): el visor de páginas renderizadas añade interacción de pan/zoom mediante la librería [panzoom](https://github.com/timmywil/panzoom) servida localmente (`wwwroot/lib/panzoom/panzoom.min.js`) tanto en `PolyglotCLI.web` como en `PolyglotCLI.Maui`. Se incluye una mini-toolbar flotante con botones Acercar/Alejar/Restablecer y un pie con atajos: arrastrar para mover, rueda del ratón para zoom, doble clic para acercar.
